...
CSV_OUTPUT_FILE="/app/ahoy_teachers_contests.csv"
...
COPY (
  ...
  WHERE name = 'teachers/contests#show'
    AND time >= '2025-09-01'
...
psql "$SCALINGO_DESTINATION_POSTGRESQL_URL" <<EOSQL
CREATE TABLE IF NOT EXISTS analytics.ahoy_teachers_contests (
...
echo "$(date '+%Y-%m-%d %H:%M:%S') - Table 'analytics.ahoy_teachers_contests' created in destination database"


# Load CSV into destination table and ensure the destination table doesn't accumulate duplicates on re-runs (with TRUNCATE)
psql "$SCALINGO_DESTINATION_POSTGRESQL_URL" <<EOSQL
BEGIN;
TRUNCATE TABLE analytics.ahoy_teachers_contests;
CREATE UNIQUE INDEX IF NOT EXISTS ahoy_teachers_contests_id_unique
  ON analytics.ahoy_teachers_contests (id);
\copy analytics.ahoy_teachers_contests FROM '$CSV_OUTPUT_FILE' WITH CSV HEADER;
COMMIT;
EOSQL

echo "$(date '+%Y-%m-%d %H:%M:%S') - Data loaded into analytics.ahoy_teachers_contests"
